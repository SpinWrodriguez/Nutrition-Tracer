...
| `src/components/AiChat.jsx` | "Coach" chat sheet (FAB). `aiDayChat` (gpt-4o, temp 0.4) runs a sports-dietitian persona with the full picture: today's meals + what is LEFT of the targets, today's exercise, this week's averages/deficit/pace, weekday vs weekend split, weight 7-day avg + 4-wk rate, waist, markers, the saved-meal list and ingredient facts (built in App.jsx `dayContext`). Starter prompt chips on an empty thread. Rules in the prompt: protein first, never suggest <~1,700 kcal or skipping meals, judge the scale by the trend, suggest from the user's own saved meals first |
